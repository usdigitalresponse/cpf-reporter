...
from src.schemas.schema_versions import Project1ARow, Project1BRow, Project1CRow
...
class TestGenerateOutput1C:
    def test_combine_project_rows(self, valid_project_sheet_1C):
        project_errors, projects = validate_project_sheet(valid_project_sheet_1C, ProjectRowSchema, V2024_05_24_VERSION_STRING)
    ...
    def test_combine_project_rows_with_conflicts_1(self, valid_project_sheet_1C, valid_project_sheet_1C_with_conflict):
        ...
        project_errors, projects = validate_project_sheet(valid_project_sheet_1C_with_conflict, ProjectRowSchema, V2024_05_24_VERSION_STRING)
        assert project_errors == []
        project_jsons = [dict(project) for project in projects]
        ...
        assert project_id_to_data.get(FIRST_ID) is not None
        assert project_id_to_data.get(FIRST_ID).Project_Name__c == "updated project 1c test"
    
    def test_combine_project_rows_with_conflicts_2(self, valid_project_sheet_1C, valid_project_sheet_1C_with_conflict):
        """Choose the project with the later created at date"""
        project_errors, projects = validate_project_sheet(valid_project_sheet_1C, ProjectRowSchema, V2024_05_24_VERSION_STRING)
        assert project_errors == []
        project_jsons = [dict(project) for project in projects]
    ...
    def test_combine_project_rows_multiple(self, valid_project_sheet_1C, valid_second_project_sheet):
        """Choose the project with the later created at date"""
        project_errors, projects = validate_project_sheet(valid_project_sheet_1C, ProjectRowSchema, V2024_05_24_VERSION_STRING)
        assert project_errors == []
        project_jsons = [dict(project) for project in projects]
        ...
        project_errors, projects = validate_project_sheet(valid_second_project_sheet, ProjectRowSchema, V2024_05_24_VERSION_STRING)
        assert project_errors == []
        project_jsons = [dict(project) for project in projects]
    ...
    def test_populate_output_report(self, valid_project_sheet_1C: Worksheet, output_template: Workbook):
        """Populate the output report with one project"""
        _, projects = validate_project_sheet(valid_project_sheet_1C, ProjectRowSchema, V2024_05_24_VERSION_STRING)
        project_jsons = [dict(project) for project in projects]
        project_id_to_data: Dict[str, Project1CRow] = {}
        project_id_to_upload_date = {}
        createdAt = datetime.now()
        ...
        populate_output_report(workbook=output_template, project_id_to_data=project_id_to_data)
        sheet = output_template['Baseline']
        assert sheet[f'B8'].value == project_id_to_data[FIRST_ID].Project_Name__c
        assert sheet[f'C8'].value == project_id_to_data[FIRST_ID].Identification_Number__c
    
    def test_populate_output_report_multiple(self, valid_project_sheet_1C, valid_second_project_sheet, output_template):
        ...
        assert sheet[f'B8'].value == project_id_to_data[FIRST_ID].Project_Name__c
        assert sheet[f'C8'].value == project_id_to_data[FIRST_ID].Identification_Number__c
        assert sheet[f'B9'].value == project_id_to_data[SECOND_ID].Project_Name__c
        assert sheet[f'C9'].value == project_id_to_data[SECOND_ID].Identification_Number__c

    def test_convert_xlsx_to_csv(self, valid_project_sheet_1C, valid_second_project_sheet, output_template):
        """Populate the output report with one project"""
        _, projects = validate_project_sheet(valid_project_sheet_1C, ProjectRowSchema, V2024_05_24_VERSION_STRING)
        project_jsons = [dict(project) for project in projects]
        project_id_to_data: Dict[str, Project1CRow] = {}
        ...
        num_rows = populate_output_report(workbook=output_template, project_id_to_data=project_id_to_data)
