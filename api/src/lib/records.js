...
async function loadRecordsForUpload(upload) {
  ...
  const workbook = await workbookForUpload(upload, {
    ...
    sheets: [
      ...
      COVER_SHEET,
      LOGIC_SHEET,
      ...Object.keys(DATA_SHEET_TYPES),
    ],
  })

  // parse certification and cover as special cases
  const [certification] = XLSX.utils.sheet_to_json(
    workbook.Sheets[CERTIFICATION_SHEET]
  )
  ...
  for (const sheetName of Object.keys(DATA_SHEET_TYPES)) {
    ...
    for (const row of rows) {
      ...
      Object.keys(row).forEach((fieldId) => {
        ...
        for (const formatter of rulesForCurrentType[fieldId]
          .persistentFormatters) {
          try {
            value = formatter(value)
          } catch (e) {
            console.log(
              `Persistent formatter failed to format value ${value} with error:`,
              e
            )
          }
        }
 ...
 */
async function recordsForUpload(upload, req = null) {
  logger.info(`recordsForUpload(${upload.id})`)

  if (req === undefined) {
    // Will not cache outside of a request
    logger.info(
      `recordsForUpload(${upload.id}) will not cache for subsequent calls`
    )
    req = {}
  }
  ...
  if (!req.recordsForUpload) {
    logger.info(
      `recordsForUpload(${upload.id}) initializing req.recordsForUpload cache`
    )
    req.recordsForUpload = {}
  }

  if (req.recordsForUpload[upload.id]) {
    logger.info(`recordsForUpload(${upload.id}): reading from cache`)
...
//   logger.info(`recordsForReportingPeriod(${periodId})`)
//   requiredArgument(
//     periodId,
//     'must specify periodId in recordsForReportingPeriod'
//   )
...
//   logger.info(`mostRecentProjectRecords(${periodId})`)
//   requiredArgument(
//     periodId,
//     'must specify periodId in mostRecentProjectRecords'
//   )

//   const reportingPeriods = await previousReportingPeriods(
//     periodId,
//     organizationId
//   )
...
//   const reportingPeriods = await getPreviousReportingPeriods(
//     periodId,
//     undefined,
//     tenantId
//   )
