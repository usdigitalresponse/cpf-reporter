...
const recordValueFormatters = {
  ...
  removeSepDashes: (val) =>
    _.isString(val) ? val.replace(/^-/, '').replace(/;\s*-/g, ';') : val,
...
function generateRules() {
  ...
  for (const ruleType of Object.keys(rules)) {
    for (const rule of Object.values(rules[ruleType])) {
      ...
      if (rule.listVals && rule.listVals.length > 0) {
        if (typeof rule === 'object') {
        ...
        for (let i = 0; i < rule.listVals.length; i += 1) {
          ...
          if (correction) {
            ...
            const valuesToCoerce = (
              correction.allowableLegacyValues || []
            ).concat(worksheetValue)

            rule.listVals[i] = correctValue
            rule.persistentFormatters.push((val) =>
              valuesToCoerce.includes(val) ? correctValue : val
            )
          }
        }
      }
...
export { getRules, dropdownCorrections }
